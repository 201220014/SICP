...
即使是这样简单的步骤，其实也揭示了一些通用过程的重要的要点。从第一步中我们可以看到，为了评估这个表达式的值，我们必须将同样的评估过程应用于它的每一个子表达式，得到每个子表达式的值。于是，我们可以很清晰地看到，这种评估的过程是一种天然的递归的过程。
...
想要评估**根节点（root）**，顶部的完整表达式，我们需要先评估每一个树枝，也就是它的子表达式。**叶子（leaf）**表达式，也就是没有子表达式的结点，一般情况下是函数或者数字。中间的结点有两个部分组成：需要应用我们上面所提到的规则的子表达式、该表达式的值。

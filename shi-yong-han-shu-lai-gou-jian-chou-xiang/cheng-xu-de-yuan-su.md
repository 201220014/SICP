...
![abs 函数](../.gitbook/assets/function\_abs.png)
...
**不纯洁的函数**：除了返回值之外，调用一个不纯洁的函数会产生**副作用（side effect）**，会改变解释器或者计算机的状态。一个常见的副作用是在返回值之外产生额外的输出，一般是使用`print`函数。
...
尽管`print`和`abs`在这两个例子里可能看起来十分相似，但它们从更本上的工作方式是不一样的。`print`函数的的返回值永远是`None`，一个特殊的Python**值（value）**，这个值就表示啥也没有的意思。对于`print`函数来说，打印输出其实是调用它所产生的一个副作用。

![print 函数](../.gitbook/assets/function\_print.png)

我们可以用一个嵌套的`print`函数调用表达式来明确地看到这个函数不纯洁的特性。
...
如果你发现这个输出结果有点出乎意料，画一个表达式树来阐明为什么评估这个表达式会产生这个奇怪的输出。小心`print`，它返回值是`None`的事实就意味着它不应当作为一个赋值语句中的表达式。
...
纯洁的函数是受严格限制的，它们不可以有任何副作用，同时，随着时间的推移，它们不应当改变自己的行为。不过，收到这些限制之后，也会有相应的好处。

1. 纯洁的函数可以被更可靠的放在复合调用表达式中。我们可以看到，在上面的不纯洁函数的例子中，我们将`print`函数当成一个操作数来使用的时候，它并不能返回一个有用的值。相比之下，我们之前提到过的`max`，`pow`，`sqrt`可以被嵌套表达式很有效的使用。
2. 纯洁的函数更容易测试。同一系列的实参永远会导向同一个返回值，可以很容易的和预计输出相比较。关于程序的测试，会在本章的后面部分讨论。
3. 第四章将会阐释，纯洁的函数对于编写**并发（concurrent）**程序十分重要，在这样的程序当中，多个调用语句可以同时被评估。相比之下，第二章将会研究一系列的不纯洁函数，并且描述它们的用处。

由于这些原因，我们在这一章的剩余部分当中将会重点聚焦于创造并使用纯洁的函数。`print`函数只在我们需要立即看到计算结果的时候使用。

{% hint style="success" %}
到此为止，读者应当对于程序的元素有了一个深刻的认识，一个高于Python语言本身的深刻的认识。
{% endhint %}
